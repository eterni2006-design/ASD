...
TEST(QueueBoolTest, BoolQueue) {
    ...
    EXPECT_TRUE(queue.front());
    EXPECT_FALSE(queue.back());

    queue.pop();
    EXPECT_FALSE(queue.front());
...
TEST(QueueBoolTest, BoolQueueEmpty) {
    Queue<bool> queue;
...
TEST(QueueBoolTest, BoolQueueLarge) {
    Queue<bool> queue;
    const int N = 100;
